...
var highStockChart = function(divID,result,crrentData){
	...
	var firstTouch = true;
	// result的顺序：
	// 0日期^1开盘价^2收盘价^3最高价^4最低价^5成交量^6成交额^7涨跌幅^8涨跌额^9换手率^10昨日收盘价^11MA5^12MA10^13MA20^14MA30^15MA60
...
//		}
		...
		var tickInterval = (( max-min)/5).toFixed(2)*1;
...
//			}], 
	    ...
	    tooltip: {
		   formatter: function() {
			   ...
			   y = (this.points[1].point.y).toFixed(2);
    		 ...
    		 /* tip += "换手率："+hsl+"<br/>";*/
    		  $reporting.html(
    				  '  <span style="font-weight:bold;padding-left:20px">'+stockName+'</span>'
    				+ '  <span>开盘:</span>'+ open
    				+'  <span>收盘:</span>'+close
              		+'  <span>最高:</span>'+ high
              		+'  <span>最低:</span>'+ low
